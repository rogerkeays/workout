...
def piece(num, title, mp3, tempo, phrases, index="-", rhythm="1234", strings="2", bowing="35",
  ...
  for i, tupl in enumerate(phrases):
    ...
    phrase(title + " @" + str(tempo), mp3, start, stop, lyrics,
...
def phrase(title, mp3, start, stop, lyrics,
  ...
  for i in range(len(index)):
    if i > 0:
      h = i - 1
      j = i + 1
      word(title, lyrics[h:j], rhythm[h:j+1], strings[h:j], shapes[h:j], bases[h:j],
           fingers[h:j], bowing[h:j+1], attack[h:j], dynamics[h:j])

  # phrase drills
  phrase_bowing(title, lyrics, index, rhythm, strings, shapes, bases, fingers, bowing, attack, dynamics)
  phrase_together(title, lyrics, index, rhythm, strings, shapes, bases, fingers, bowing, attack, dynamics)

def phrase_bowing(title, lyrics, index, rhythm, strings, shapes, bases, fingers, bowing, attack, dynamics):
  make_card(locals(), 5)

def phrase_together(title, lyrics, index, rhythm, strings, shapes, bases, fingers, bowing, attack, dynamics):
  make_card(locals(), 5)

def word(title, lyrics, rhythm, strings, shapes, bases, fingers, bowing, attack, dynamics):

  # process notes
  note(title, lyrics[0], rhythm[0:2], strings[0], shapes[0], bases[0], fingers[0],
       bowing[0:2], attack[0], dynamics[0])
  note(title, lyrics[1], rhythm[1:3], strings[1], shapes[1], bases[1], fingers[1],
       bowing[1:3], attack[1], dynamics[1])

  # word drills
  word_bowing(title, lyrics, rhythm, strings, shapes, bases, fingers, bowing, attack, dynamics)
  word_together(title, lyrics, rhythm, strings, shapes, bases, fingers, bowing, attack, dynamics)

def word_bowing(title, lyrics, rhythm, strings, shapes, bases, fingers, bowing, attack, dynamics):
  params = locals()
  make_card(params, 5)

def word_together(title, lyrics, rhythm, strings, shapes, bases, fingers, bowing, attack, dynamics):
  params = locals()
  make_card(params, 5)

def note(title, lyrics, rhythm, string, shape, base, finger, bowing, attack, dynamic):
  note_bowing(title, lyrics, rhythm, string, shape, base, finger, bowing, attack, dynamic)
  note_together(title, lyrics, rhythm, string, shape, base, finger, bowing, attack, dynamic)

def note_bowing(title, lyrics, rhythm, string, shape, base, finger, bowing, attack, dynamic):
  params = locals()
  make_card(params, 5)

def note_together(title, lyrics, rhythm, string, shape, base, finger, bowing, attack, dynamic):
  params = locals()
  ...
  fret = int(base) + frets[int(finger) - 1]
