...
def violin(number, name, video_id, meter, tempo, tonic, sections, speeds=[0.5, 1.0], video=True):
  process_piece(Piece("violin", number, name, video_id, meter, tempo, tonic, sections, speeds, video),
...
def make_violin_drill(params, reps=5):
  make_drill("violin", params, reps)

def make_violin_drone(params):
  make_drone("violin", note)
...
def air_hammers(finger):
  make_violin_drill(locals(), 30)

def arm_stretches():
  make_violin_drill(locals(), 1)

def beat_clapping(tempo, rhythm):
  make_violin_drill(locals(), 5)
...
def finger_hammers(string, fret, finger):
  air_hammers(finger)
  make_violin_drill(locals(), 15)

def finger_stretches():
  make_violin_drill(locals())
...
def hand_placement(string, shape, base):
  if shape in SHAPES:
    ...
    if make_violin_drill(locals(), 60):
      make_violin_drone(note_at(string, base))
...
def pitch_hitting(string, fret, finger):
  if int(fret) != 0 and int(finger) != 0:
    ...
    tuning()
    if make_violin_drill(locals(), 5):
      make_violin_drone(note_at(string, fret))

def son_file():
  make_metronome(60)
  make_violin_drill(locals(), 4)
...
def tuning():
  if make_violin_drill(locals(), 1):
    make_violin_drone("49")
